...
def L0Deblur_dark_channel(Im, kernel, lambda_, wei_grad, kappa=2.0):
    ...
    S_squared = (S*S).cpu().detach().numpy()
    
    # Görüntünün boyutu 3 boyutlu ise (RGB görüntü), griye çevir
    if S_squared.ndim == 3 and S_squared.shape[2] > 1:
        S_squared = cv.cvtColor(S_squared, cv.COLOR_RGB2GRAY)
    
    # Değerleri 0-255 aralığına normalize et
    S_squared = (S_squared - S_squared.min()) / (S_squared.max() - S_squared.min() + 1e-7) * 255
    S_squared = S_squared.astype('uint8')
    
    # Threshold işlemini uygula
    ret2, th2 = cv.threshold(S_squared, 0, 255, cv.THRESH_BINARY+cv.THRESH_OTSU)
